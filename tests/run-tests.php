 ...
 *
 * To get a list of all @group tags: phpunit --list-groups Soliant/
 *
    ...
    for ($i = 1; $i < $argc; $i++) {
        ...
        switch ($arg) {
            ...
            case 'all':
                ...
                break;
            default:
                if (strpos($arg, 'Soliant') !== false) {
                    $components[] = $arg;
                } else {
                    $components[] = 'Soliant_' . $arg;
                }
        }
    }
}
...
if ($run_as == 'groups') {
    ...
    foreach ($components as $component) {
        $component =   'SoliantTest/' . basename(str_replace('_', '/', $component));
        echo "$component:\n";
...
function getAll($phpunit_conf)
{
    ...
    if ($handle = opendir(__DIR__ . '/SoliantTest/')) {
